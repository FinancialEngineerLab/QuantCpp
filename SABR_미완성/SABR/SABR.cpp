...
double SABRIV(double alpha, double beta, double v, double rho, double Fut, double K, double T)
{
	...
	double FKTerm = pow(F * K, 1.0 - beta);
	double sqrtFKTerm = sqrt(FKTerm);
	double SABRIV;
	z = (v / alpha) * sqrtFKTerm * log(F / K);
	x = log( (sqrt(1.0 - 2.0 * rho * z + z * z  ) + z - rho)/(1.0-rho)   )  ;
	numerator = ((1.0 - beta) * (1.0 - beta) / 24.0) * ((alpha * alpha) / (FKTerm)) + 0.25 * (rho * beta * v * alpha) / (sqrtFKTerm + (((2.0 - 3.0 * rho * rho) * v * v) / 24.0));
	denominator = (sqrtFKTerm * (1.0 + (((1.0 - beta) * (1.0 - beta)) / 24.0) * (log(F / K) * log(F / K)) + (((1.0 - beta) * (1.0 - beta) * (1.0 - beta) * (1.0 - beta)) / 1920.0) * (log(F / K) * log(F / K) * log(F / K) * log(F / K))));
	
	if (x != 0) SABRIV = (alpha * (1.0 + numerator) * T) / denominator * z / x;
	else SABRIV = (alpha * (1.0 + numerator) * T) / denominator;
	
	if (F == K) SABRIV = (alpha * (1.0 + numerator) * T) / denominator;
...
void Levenberg_Marquardt_SABR(long NParams, long NResidual, double* NextParams, double* CurrentParams, double* lambda, double** Jacov, double* Residual, double& ParamSum, double** JT_J, double** Inverse_JT_J, double** JT_Res, double** ResultMatrix)
{
	long i;
	double mu = *lambda;
	double alphamax = 2.0, alphamin = 0.00001;
	double vmax = 2.0, vmin = 0.00001;
	double rhomax = 1.0, rhomin = -1.0;
...
int main()
{
	...
	double Fut = 1.0;
	double ResultParams[3] = {alpha,  v, rho };
	double Beta = 1.0;
	main2(NTermVol, TermVol, NParityVol, ParityVol, Vol, Fut, Beta, ResultParams, ResultLocVol);
}